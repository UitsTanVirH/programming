...
int ccw(point o, point a, point b){
    point p=a-o, q=b-o;                // oa vector and ob vector
    if((p^q)>0)             return +1; // counter clockwise
    if((p^q)<0)             return -1; // clockwise
    if((p*q)<0)             return +2; // b - o - a , a and b is in opposite position
    if(a.norm() < b.norm()) return -2; // o -- a -- b, o and b is in opposite position
    return 0;  // o -- b -- a , o and a is in opposite position
}
